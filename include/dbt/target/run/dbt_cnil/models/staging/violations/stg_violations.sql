    ...
    create or replace table `cnil-392113`.`dev_dbt_staging`.`stg_violations`
    
    

    OPTIONS()
    as (
      SELECT PARSE_DATE('%Y-%m', date_de_reception_de_la_notification) as date_date,
...
FROM `cnil-392113`.`notifications_a_la_cnil_de_violations_de_donnees_a_caractere_personnel`.`opencnil_violationsdcpnotifiees_20230331`
WHERE date_de_reception_de_la_notification NOT LIKE '%Date de réception de la notification%'
    );
